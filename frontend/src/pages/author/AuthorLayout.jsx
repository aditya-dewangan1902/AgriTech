...
import { Menu, LayoutDashboard, UploadCloud, User, Home, LogOut } from 'lucide-react';
...
const AuthorLayout = () => {
  ...
  const navItems = [
    ...
    { to: '/author/new-submission', label: 'New Submission', icon: UploadCloud },
    { to: '/author/profile', label: 'Profile / Settings', icon: User },
  ];

  const getNavClass = (path) => {
    const base = 'flex items-center space-x-3 p-3 rounded-lg transition-all duration-200 font-medium';
    const isActive = location.pathname === path || location.pathname.startsWith(path + '/');
    if (isActive) {
      // Author active: green background with green-400 right border accent (from author_style.css)
      return `${base} bg-[#006633] text-white border-r-4 border-[#34d399] font-semibold`;
    }
    ...
    <div className="flex overflow-x-hidden" style={{ backgroundColor: '#f3f4f6' }}>
      ...
      <header className="fixed top-0 left-0 w-full bg-white z-30 lg:hidden flex justify-between items-center h-16 border-b border-[#E5E5E5] px-4">
        <Link to="/" className="flex items-center space-x-2">
          ...
          <span className="merriweather text-xl font-bold text-gray-800">Author Portal</span>
        </Link>
      ...
      >
        ...
        <div className="p-6 pt-8 flex items-center space-x-2 pb-4 border-b border-green-700 mb-2">
          ...
          <h2 className="text-2xl font-bold merriweather text-white">Author Portal</h2>
        </div>
      ...
      {/* Main Content Area */}
      <main className="flex-grow min-h-screen pt-16 lg:pt-0" style={{ marginLeft: '256px' }}>
